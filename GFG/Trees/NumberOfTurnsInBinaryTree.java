...
class GFG {
  ...
  public static void main(String[] args) throws IOException {
    ...
    while (t > 0) {
      ...
      Tree g = new Tree();
      if ((turn = g.NumberOfTurns(root, nd1, nd2)) != 0) System.out.println(
        turn
...
class Tree {
  ...
  static int turnsRecur(Node node, int a, int b, int count, char direction) {
    ...
    if (node.data == a || node.data == b) return count;
    ...
    if (direction == 'l') {
      ...
      return (left != -1)
        ? left
        : turnsRecur(node.right, a, b, count, direction);
    }
  }
  ...
  static int NumberOfTurns(Node root, int first, int second) {
    ...
    if (lca.data == first) {
      ...
      turns =
        (turns != -1) ? turns : turnsRecur(lca.right, second, second, 0, 'r');
      ...
      turns =
        (turns != -1) ? turns : turnsRecur(lca.right, first, first, 0, 'r');
