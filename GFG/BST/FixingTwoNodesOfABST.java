...
class GfG {
  ...
  public static boolean compare(Node a, Node b, ArrayList<pair> mismatch) {
    ...
    return (
      compare(a.left, b.left, mismatch) && compare(a.right, b.right, mismatch)
    );
  }
...
class Sol {
  ...
  void correctBSTRecur(Node node) {
    if (node == null) return;

    correctBSTRecur(node.left);

    if (first == null && prev != null && prev.data > node.data) first = prev;

    if (first != null && prev.data > node.data) second = node;

    prev = node;

    correctBSTRecur(node.right);
  }

  public Node correctBST(Node root) {
    //code here.

    correctBSTRecur(root);
