...
class GfG {
  ...
  static Node deleteNode(Node node, int key) {
    ...
    if (key > node.data) node.right = deleteNode(node.right, key); else if (
      key < node.data
    ) node.left = deleteNode(node.left, key); else {
      if (node.right == null) return node.left;
  ...
  static Node addNode(Node node, int key) {
    ...
    if (key > node.data) node.right = addNode(node.right, key); else if (
      key < node.data
    ) node.left = addNode(node.left, key); else {
