...
class Solution{
    ...
    static Info largestBSTRecur(Node node){
        ...
        if (node.left == null && node.right == null)
            return new Info(true, 1, node.data, node.data);
        ...
        if (left.isBST == false || right.isBST == false)
            return new Info(false, Math.max(left.count, right.count), 0, 0);
        
        // both childs are BST
        // check for root
        
        if (left.max < node.data && right.min > node.data)
            return new Info(true, left.count + right.count + 1, left.min, right.max);
        else
            return new Info(false, Math.max(left.count, right.count), 0, 0);
